...
if(isset($_POST['submit']) 
	...
	elseif (strpos($q,'*')){
		$_q = str_replace('*','%',$q);
		$sql = 'SELECT * FROM '.$tblprefix.$d.' WHERE kanji LIKE \''.$_q.'\' LIMIT '.DICT_LIST_NUM;
		$fmt = 'html';
	} else {
		$sql = 'SELECT * FROM '.$tblprefix.$d.' WHERE kanji=\''.$q.'\'';
	}
	$res = mysql_query($sql,$db);
	//echo get_resource_type ( $res );
	if($res){
		$row  = ( $id || $_q ) ? mysql_fetch_assoc($res) : fetchRow($res,$q);
		$query = isset($query) ? $query : $row['kanji'];
		switch(strtolower($f)){
			case 'context':
...
$form[] = '<input id="bSubmit" type="submit" name="submit" value="搜索">';
...
if (!isset($result) && isset($word) ) {
	$content = '<h3>Kh?ng tìm th?y t? yêu c?u!</h3>';
}
